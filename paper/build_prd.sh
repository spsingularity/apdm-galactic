#!/usr/bin/env bash
# Build the Physical Review D PDF (revtex4-2, numeric natbib) for Paper VIII
# from APDM_galactic.md. Pipeline identical to build_oja.sh; only the template,
# keyword separator and bibliography style differ. PRD is numeric, so \citet is normalised to \citep.
set -e
cd "$(dirname "$0")"
mkdir -p tex
BASE=APDM_galactic
...
pandoc -f markdown-superscript-subscript .build.md -o tex/$BASE.tex \
  --standalone --shift-heading-level-by=-1 --natbib --template=tools/template_prd.tex
...
# PRD is a numeric-citation journal: no author-prominent form
