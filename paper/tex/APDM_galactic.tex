...
\citet{Limbach:2008aa} (2008), who reached the marginally
...
\end{abstract}
\keywords{modified Newtonian dynamics; galaxy rotation curves; dark
energy; emergent gravity; radial acceleration relation}
...
by \citet{Limbach:2008aa}, who defined the same two evolutionary
...
The USC entropy clock is \(\mathcal{D}\)\_E = \(\Theta\)̇\_E/H =
...
0.33 \(\to\) 1.44 (script \texttt{branch\_discrimination\_test.py},
template quantiles from \texttt{desi\_template\_quantiles.csv}):
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{figures/fig1_branch_discrimination.png}}
...
\texttt{branch\_discrimination\_test.py}).
...
(\texttt{muse\_release\_galaxies.csv}, 85 numeric candidates) using the
raw outer rotational acceleration a\_rot \(\sim\) V\(^{2}\)/r (script
\texttt{mass\_controlled\_arot\_trend.py}); the result remains open.
Three facts emerged:
...
footnote to a live scientific issue (the decisive follow-up, E-1″, is in
§\ref{sec-7}). This is the one place where USC's own data-hygiene
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{figures/fig2_rar.png}}
...
response inside SEDE gives (script \texttt{d3prime\_emergent\_mond.py}),
...
\texttt{d3prime\_thermal\_consistency.py}) --- far below one, so we
...
\texttt{d4prime\_worldline\_kernel.py}):

\begin{quote}
\textbf{\(\int\)d\(\Delta\tau\) cosh(a\(\Delta\tau\))
G\(^{+}\)(\(\Delta\tau\)) =
(a/4\(\pi\))\(\cdot\)cot(\(\pi\)a/\(\kappa\))}, \(\kappa\) =
\surd (a\(^{2}\)+H\(^{2}\)), and the second-order rate \(\Sigma\)̇(a) =
...
\item
  \textbf{(iv) Deep-a limit:} \(\Sigma\)̇ \(\to\)
...
\item
  \textbf{(v) Large-a pathology:} \(\Sigma\)̇ \(\to\)
...
falsifier matrix (Zenodo \texttt{10.5281/zenodo.21415326}). We restate
...
the declining high-z rotation curves of \citet{Genzel:2017jgd},
\citet{Milgrom:2017hpv} constrained the evolution of a\(_{0}\) and found
...
clean in either direction: \citet{Mayer:2022qhk} find, in
...
\url{https://github.com/spsingularity/apdm-galactic}:

\begin{itemize}
\tightlist
\item
  \texttt{branch\_discrimination\_test.py} (E-1): the two-branch
  templates over z = 0.33 \(\to\) 1.44; density +16.7\(\sigma\), rate
  +4.4\(\sigma\); the MUSE-preferred power a\(_{0}\) \(\propto\)
  H\^{}\{1.23\}. Data: \texttt{desi\_template\_quantiles.csv},
  \texttt{muse\_release\_galaxies.csv}, MUSE-DARK III published
  a\(_{0}\)(z) = (1.00 \(\pm\) 0.04) + (1.59 \(\pm\) 0.11)z.
\item
  \texttt{d3prime\_emergent\_mond.py} (D-3′): the exact gated-Verlinde
  identity; the elastic/activated/density sub-branch exclusions
  (19.0\(\sigma\) / 27.3\(\sigma\) / 19.5\(\sigma\)); the KMS-tilt RAR
  pass at 0.057 dex.
\item
  \texttt{d3prime\_thermal\_consistency.py}: the \textasciitilde3 ×
  10\(^{-22}\) galactic bath-mode count that forces the vacuum framing.
\item
  \texttt{d4prime\_worldline\_kernel.py} (D-4′): the master integral
...
\item
  \texttt{mass\_controlled\_arot\_trend.py} (E-1′): the raw-catalogue
...
\texttt{10.5281/zenodo.21415326}; nothing in it may be edited after
...
\item
  Pre-registration freeze: Zenodo \texttt{10.5281/zenodo.21415326}.
\item
